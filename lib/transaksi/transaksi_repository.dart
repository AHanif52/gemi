  ...
  Future<List<TransaksiBaris>> semua() async {
    ...
    return [
      for (final r in rows)
        TransaksiBaris(
          r.readTable(t),
          kategori: r.readTableOrNull(k),
          kantong: r.readTable(a),
          kantongTujuan: r.readTableOrNull(tujuan),
        ),
    ];
  }

  Future<int> tambah(TransactionsCompanion data) =>
      _db.into(_db.transactions).insert(data);
